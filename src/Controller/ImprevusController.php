...
{
    private function isMissingCasRellesSchema(\Throwable $exception): bool
    {
        $current = $exception;
        while ($current !== null) {
            $message = strtolower($current->getMessage());
            if (str_contains($message, 'confirmed_at')) {
                return true;
            }
            if (str_contains($message, 'cas_relles') && str_contains($message, 'unknown column')) {
                return true;
            }
    ...
    private function safeFindCasRelles(
        CasRellesRepository $repo,
        array $criteria,
        array $orderBy = [],
        ?User $userForFlash = null
    ): array {
        try {
            return $repo->findBy($criteria, $orderBy);
        } catch (\Throwable $e) {
            if (!$this->isMissingCasRellesSchema($e)) {
                throw $e;
            }
            if ($userForFlash instanceof User) {
                $this->addFlash('warning', 'Case history is temporarily unavailable (database schema not up to date).');
            }
            return [];
        }
    }

    private function isMissingUserNotificationTable(\Throwable $exception): bool
    {
        $current = $exception;
        while ($current !== null) {
            if ($current instanceof TableNotFoundException) {
                return true;
            }
            if (str_contains(strtolower($current->getMessage()), 'user_notification')) {
                return true;
            }
            $current = $current->getPrevious();
        }
    ...
    ): Response
    {
        ...
        if ($user instanceof User) {
            try {
                ...
                if (!$this->isMissingUserNotificationTable($e)) {
                    throw $e;
                }
                $refusalPopup = null;
            }
        ...
        if ($user instanceof User) {
            $userCases = $this->safeFindCasRelles($casRellesRepository, ['user' => $user], ['dateEffet' => 'DESC'], $user);
    ...
    ): Response
    {
        ...
        $user = $this->getUser();
        $cas = $this->safeFindCasRelles($repo, ['user' => $user], ['dateEffet' => 'DESC'], $user instanceof User ? $user : null);
        $notifications = [];
        try {
    ...
    public function adminHistoryCopy(CasRellesRepository $repo): Response
    {
        ...
        $cas = $this->safeFindCasRelles($repo, ['user' => $user, 'resultat' => 'VALIDE'], ['dateEffet' => 'DESC'], $user instanceof User ? $user : null);
