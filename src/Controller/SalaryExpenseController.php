...
{
    private function isMissingRecurringTable(\Throwable $exception): bool
    {
        $current = $exception;
        while ($current !== null) {
            ...
            if (str_contains(strtolower($current->getMessage()), 'recurring_transaction_rule')) {
                return true;
            }
    ...
    public function index(
        ...
        try {
            ...
            if (!$this->isMissingRecurringTable($e)) {
                throw $e;
            }
            $this->addFlash('warning', 'Recurring rules are temporarily unavailable (database table missing).');
        }
    ...
    public function acceptRecurringSuggestion(
        ...
        try {
            ...
            $this->addFlash('warning', 'Recurring rules table is missing. Run database migrations first.');
            return $this->redirectToRoute('app_salary_expense_index', ['tab' => 'expenses']);
        }
